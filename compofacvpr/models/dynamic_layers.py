...
from ..utils import int2list, get_net_device
...
class DynamicResConvLayer(MyModule):
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        # conv2
        importance = torch.sum(torch.abs(self.conv2.conv.conv.conv.weight.data), dim=(0, 2, 3))
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        self.conv2.conv.conv.conv.weight.data = torch.index_select(
        ...
        importance = torch.sum(torch.abs(self.conv1.conv.conv.conv.weight.data), dim=(0, 2, 3))
        ...
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        self.conv1.conv.conv.conv.weight.data = torch.index_select(
            self.conv1.conv.conv.conv.weight.data, 1, sorted_idx
        )

        if self.downsample:
            importance = torch.sum(torch.abs(self.downsample.conv.conv.conv.weight.data), dim=(0, 2, 3))
            sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
            self.downsample.conv.conv.conv.weight.data = torch.index_select(
