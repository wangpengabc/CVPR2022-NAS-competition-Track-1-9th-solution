...
from ..utils import SingleMachineMetric, DistributedMetric, list_mean, cross_entropy_loss_with_soft_target, \
...
def validate(run_manager, epoch=-1, is_test=True, image_size_list=None,
    ...
    if ks_list is None:
        ks_list = [None] if dynamic_net.fixed_kernel else dynamic_net.ks_list
    if expand_ratio_list is None:
        expand_ratio_list = dynamic_net.expand_ratio_list
    if depth_list is None:
        depth_list = dynamic_net.depth_list
    # if hvd.rank() == 0:
    print("expand_ratio_list: ", expand_ratio_list)

    subnet_settings = []
    if dynamic_net.compound:
        if len(expand_ratio_list)==1:
            expand_ratio_list = [expand_ratio_list[0]]*len(depth_list)
        for w in width_mult_list:
            for d, e in zip(depth_list, expand_ratio_list):
                for k in ks_list:
    ...
    for setting, name in random.sample(subnet_settings, 5):
        ...
        run_manager.log_to_tensorboard(f'top1/val/{name}', top1, epoch)
        run_manager.log_to_tensorboard(f'loss/val/{name}', loss, epoch)
...
def train_one_epoch(run_manager, args, epoch, warmup_epochs=0, warmup_lr=0):
    ...
    dynamic_net = run_manager.net
    if isinstance(dynamic_net, nn.DataParallel):
        dynamic_net = dynamic_net.module # nn.DataParallel将model转换成了model.module
    ...
    losses = SingleMachineMetric('train_loss')
    top1 = SingleMachineMetric('train_top1')
    top5 = SingleMachineMetric('train_top5')
...
def supporting_elastic_depth_cvpr_resnet(train_func, run_manager, args, validate_func_dict):
    dynamic_net = run_manager.net
    if isinstance(dynamic_net, nn.DataParallel):
        dynamic_net = dynamic_net.module
    ...
    depth_stage_list = dynamic_net.depth_list.copy()    # depth_list : [[2,3,4,5], [2,3,4,5,6,7,8]]
    # depth_stage_list.sort(reverse=True)
    # n_stages = len(depth_stage_list) - 1
    # start_stage = n_stages - 1
    depth_stage_124 = depth_stage_list[0].copy()
    depth_stage_3 = depth_stage_list[1].copy()
    ...
    # )
    run_manager.write_log(
        '-' * 30 + 'Supporting Elastic Depth: stage124: {}  stage3: {} '.format(depth_stage_124, depth_stage_3)
         + '-' * 30, 'valid'
    )
...
def supporting_elastic_expand(train_func, run_manager, args, validate_func_dict):
    ...
    if not args.resume:
        if args.phase == 1:
            ...
            #                           model_dir='.torch/ofa_checkpoints/%d' % hvd.rank())
            model_path = os.path.join(run_manager.path.replace('depth2depth_width/phase1','base2depth/phase2'), 'checkpoint/model_best.pth.tar')
            load_models(run_manager, dynamic_net, model_path=model_path)
        else:
...
def supporting_elastic_expand_cvpr_resnet(train_func, run_manager, args, validate_func_dict):
    dynamic_net = run_manager.net
    if isinstance(dynamic_net, nn.DataParallel):
        dynamic_net = dynamic_net.module
    ...
    if not args.resume:
        if args.phase == 1:
            # model_path = download_url('https://hanlab.mit.edu/files/OnceForAll/ofa_checkpoints/ofa_D234_E6_K357',
            #                           model_dir='.torch/ofa_checkpoints/%d' % hvd.rank())
            # model_path = os.path.join(run_manager.path.replace('depth2depth_width/phase1', 'base2depth/phase2'),
...
def supporting_compound(train_func, run_manager, args, validate_func_dict):
    dynamic_net = run_manager.net
    if isinstance(dynamic_net, nn.DataParallel):
        dynamic_net = dynamic_net.module
    ...
    run_manager.write_log(
        '-' * 30 + 'Supporting Compound Depth+Width' + '-' * 30, 'valid'
    )
    dynamic_net.clear_constraint()
    # train
    train_func(
        run_manager, args,
        lambda _run_manager, epoch, is_test: validate(_run_manager, epoch, is_test, **validate_func_dict)
    ...
    # save and validate
    run_manager.save_model(model_name='compound.pth.tar')
    run_manager.write_log('%.3f\t%.3f\t%.3f\t%s' % validate(run_manager, **validate_func_dict), 'valid')


def supporting_without_ps(train_func, run_manager, args, validate_func_dict):
    dynamic_net = run_manager.net
    if isinstance(dynamic_net, nn.DataParallel):
        dynamic_net = dynamic_net.module
    ...
    run_manager.write_log(
        '-' * 30 + 'Supporting Without PS' + '-' * 30, 'valid'
    )
    dynamic_net.clear_constraint()
    # train
    train_func(
    ...
    # save and validate
    run_manager.save_model(model_name='without-ps.pth.tar')
    run_manager.write_log('%.3f\t%.3f\t%.3f\t%s' % validate(run_manager, **validate_func_dict), 'valid')
