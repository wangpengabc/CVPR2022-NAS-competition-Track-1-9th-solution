...
from .layers_utils import get_same_padding
from .layers_utils import make_divisible
from .basic_layers import SEModule
from ..utils import sub_filter_start_end


class DynamicSeparableConv2D(nn.Layer):
    """
    Separatble Conv, group = in_channel, reference: https://pytorch.org/docs/stable/generated/torch.nn.Conv2D.html
    """
    ...
    def __init__(self, max_in_channels, kernel_size_list, stride=1, dilation=1):
        ...
        self.conv = nn.Conv2D(
            ...
            groups=self.max_in_channels, bias_attr=False,
        )
...
class DynamicPointConv2D(nn.Layer):
    """
    element-wise conv.
    In ofa_mbv3, kernel_size=1, stride=1, dialation=1. Used to make the output channel equal to next layer's input channel.
    """
    ...
    def __init__(self, max_in_channels, max_out_channels, kernel_size=1, stride=1, dilation=1):
        ...
        self.conv = nn.Conv2D(
            self.max_in_channels, self.max_out_channels, self.kernel_size, stride=self.stride, bias_attr=False,
        )
...
class DynamicBatchNorm2d(nn.Layer):
